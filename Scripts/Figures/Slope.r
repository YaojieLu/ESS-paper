...
plot(datab$P50, datab$sigma,
     ...
     xlim=c(-10, 0), ylim=c(0, 1.5), cex.lab=1.3, pch=1, lwd=1, col="red")

lines(datab$P50, predict(fit), lwd=1, col="red")

points(subset(dataDVSI, h3==25 & d<11, select=c("P50", "slope")), type="l", lwd=4)
points(subset(dataDVSII, h3==25,select=c("P50", "slope")), type="l", lwd=2)
segments(subset(dataDVSI, h3==25 & d==1, select=c("P50"))[[1]], 0,
         subset(dataDVSI, h3==25 & d==10, select=c("P50"))[[1]], 0, lwd=1)
## V
...
legend("topright", legend=expression(Martinez-vilalta~italic(et~al.)~2014), col="red", lty=1, lwd=1, pch=1)
box()
