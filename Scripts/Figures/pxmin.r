...
points(dataAng, type="p", col=Cols[1], pch=1, lwd=1)
lines(dataAng$Psi50, predict(fitAng), col=Cols[1], lwd=1)
points(dataGym, type="p", col=Cols[2], pch=2, lwd=1)
lines(dataGym$Psi50, predict(fitGym), col=Cols[2], lwd=1)

points(subset(dataDVSIII, d<11, select=c("P50", "pxmin")), type="l", lwd=1)
points(subset(dataDVSI, h3==1 & d<11, select=c("P50", "pxmin")), type="l", lwd=4)
points(subset(dataDVSI, h3==100 & d<11, select=c("P50", "pxmin")), type="l", lty=2, lwd=4)
...
mtext(expression(psi[xmin]~(MPa)),side=2,line=2, cex=1.3)
text(-15*0.965, -15*0.05, "c", cex=1.3)
legend("bottomleft", title=expression(Choat~italic(et~al.)~2012), c("Angiosperm", "Gymnosperm"), lwd=1, pch=c(1, 2), lty=c(1, 1), col=Cols[1:2])
box()
